...
pub fn generate_html_report(
    ...
    let severity_counts = verified_findings
        .iter()
        .fold(HashMap::new(), |mut acc, finding| {
            let severity_level = finding
                .finding
                .riskdesc
                .split_whitespace()
                .next()
    ...
    writeln!(
        file,
        "body {{ background-color: #343a40; color: #ffffff; font-family: 'Roboto', sans-serif; }}"
    )?;
    writeln!(file, ".container-fluid {{ padding-top: 20px; }}")?;
    writeln!(file, ".card {{ background-color: #495057; }}")?;
    writeln!(
        file,
        ".card-header, .table {{ background-color: #6c757d; }}"
    )?;
    writeln!(
        file,
        ".card-title, .card-text, th, td {{ color: #ffffff; }}"
    )?;
    ...
    writeln!(
        file,
        ".card-header {{ background-color: var(--primary-color); color: var(--dark-text); }}"
    )?;
    writeln!(
        file,
        ".response-section {{ margin-top: 20px; color: #ccc; }}"
    )?;
    writeln!(
        file,
        ".response-header, .response-value {{ word-wrap: break-word; color: #ccc; }}"
    )?;
    writeln!(
        file,
        ".response-table {{ margin-bottom: 20px; border: 1px solid #777; }}"
    )?;
    writeln!(
        file,
        ".response-table th, .response-table td {{ border: 1px solid #777; }}"
    )?;
    ...
    writeln!(
        file,
        "<thead><tr><th>Severity</th><th>Count</th></tr></thead><tbody>"
    )?;
    ...
    for (i, verified_finding) in sorted_findings.iter().enumerate() {
        ...
        writeln!(
            file,
            "<a href='#finding{}' class='list-group-item list-group-item-action toc-item'>{}</a>",
            i,
            format!("{} - {}", verified_finding.finding.name, severity)
        )?;
    }
    writeln!(file, "</div>")?;
    for (i, verified_finding) in sorted_findings.iter().enumerate() {
        ...
        if !verified_finding.wstg_category.is_empty() {
            writeln!(
                file,
                "<h6>WSTG Category: {}</h6>",
                verified_finding.wstg_category
            )?;
        }
        writeln!(file, "</div>")?;
        ...
        writeln!(
            file,
            "<p class='card-text'><strong>Verified:</strong> <span class='{}'>{}</span></p>",
            if verified_finding.verified {
                "verified-yes"
            } else {
                "verified-no"
            },
        ...
        writeln!(
            file,
            "<p class='card-text'><strong>Risk:</strong> <span class='{}'>{}</span></p>",
        ...
        let uris_list = verified_finding
            .finding
            .uris
            .iter()
        ...
        writeln!(
            file,
            "<p class='card-text'><strong>Affected Assets:</strong></p><ul>{}</ul>",
            uris_list
        )?;
        ...
        if let Some(request) = &verified_finding.request {
            writeln!(
                file,
                "<h6>Request</h6><pre><code>{}</code></pre>",
                escape_html(request)
            )?;
        }
