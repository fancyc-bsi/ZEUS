...
impl ZapClient {
    ...
    pub async fn get_firefox_binary(&self) -> Result<String, ZapError> {
        ...
        match json["FirefoxDriverPath"].as_str() {
            ...
            None => Err(ZapError::Other(
                "Failed to get Firefox driver path".to_string(),
            )),
        }
    }
    ...
    pub async fn run_automation_plan(&self, config_path: &str) -> Result<String, ZapError> {
        ...
        let run_plan_url = format!(
            "{}/JSON/automation/action/runPlan/?filePath={}",
            self.base_url, absolute_path
        );
        ...
        match plan_id {
            ...
            _ => Err(ZapError::Other(
                "Failed to extract plan ID from response.".to_string(),
            )),
        }
    }
    ...
    pub async fn check_plan_progress(&self, plan_id: &str) -> Result<String, ZapError> {
        let progress_url = format!(
            "{}/JSON/automation/view/planProgress/?planId={}",
    ...
    pub async fn set_http_proxy(&self, host: &str, port: &str) -> Result<String, ZapError> {
        let set_proxy_url = format!(
            "{}/JSON/network/action/setHttpProxy/?host={}&port={}",
            self.base_url, host, port
        );

        self.client.get(&set_proxy_url).send().await?;

        let enable_proxy_url = format!(
            "{}/JSON/network/action/setHttpProxyEnabled/?enabled=true",
            self.base_url
        );

        let response = self
            .client
            .get(&enable_proxy_url)
            .send()
            .await?
            .text()
            .await?;
    ...
    pub async fn flush_http_proxy(&self) -> Result<String, ZapError> {
        ...
        let disable_proxy_url = format!(
            "{}/JSON/network/action/setHttpProxyEnabled/?enabled=false",
            self.base_url
        );

        let response = self
            .client
            .get(&disable_proxy_url)
            .send()
            .await?
            .text()
            .await?;
    ...
    pub async fn start_ajax_spider(&self, target_url: &str) -> Result<String, ZapError> {
        ...
        if !response.status().is_success() {
            return Err(ZapError::Other(format!(
                "AJAX Spider request returned with status: {}",
                response.status()
            )));
        }
    ...
    pub async fn spider_status(&self, scan_id: &str) -> Result<String, ZapError> {
        let status_url = format!(
            "{}/JSON/spider/view/status/?scanId={}",
    ...
    pub async fn start_scan(&self, target_url: &str) -> Result<String, ZapError> {
        ...
        let scan_id = json.get("scan").and_then(Value::as_str).ok_or_else(|| {
            error!(
                "Error: Scan ID not found in response. Response text: {}",
                text
            );
    ...
    pub async fn scan_status(&self, scan_id: &str) -> Result<String, ZapError> {
        ...
        let status = json
            ...
            .ok_or_else(|| ZapError::Other("Status not found in response".to_string()))?;
        Ok(status.to_string())
    }
    ...
    pub async fn fetch_and_save_json_report(
        &self,
        ...
        fs::create_dir_all(project_folder_path)
            .map_err(|e| ZapError::Other(format!("Failed to create directory: {}", e)))?;

        let absolute_path = fs::canonicalize(project_folder_path)
            .map_err(|e| ZapError::Other(format!("Failed to resolve absolute path: {}", e)))?;
        ...
        if final_file_path.exists() {
            ...
            Err(ZapError::Other(
                "Report file does not exist in the expected location".to_string(),
            ))
        }
    }

    pub async fn fetch_and_save_html_report(
        &self,
        project_folder_path: &PathBuf,
        report_title: &str,
        description: &str,
