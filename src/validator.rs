...
impl Validator {
    pub fn new(proxy: Option<String>) -> Self {
        ...
        let client = if let Some(proxy_url) = proxy {
            client_builder.proxy(Proxy::all(proxy_url).unwrap()).build().unwrap()
        } else {
    ...
    pub async fn validate_target_url(&self, url: &str) -> bool {
        match self.client.get(url).send().await {
            ...
            Ok(resp) => {
                error!("Target URL returned non-success status {}: {}", url, resp.status());
                false
            },
            Err(e) => {
    ...
    fn check_yaml(&self, contents: &str, path: &str) -> bool {
        match YamlLoader::load_from_str(contents) {
            Ok(docs) => {
                info!("Checking if YAML configuration file is syntactically correct: {}", path);
    ...
    fn validate_env(&self, doc: &Yaml) -> bool {
        ...
        for context in env.unwrap() {
            if context["urls"].as_vec().map_or(true, |urls| urls.is_empty()) {
    ...
    fn check_ports_in_use() -> bool {
        ...
        for port in ports.iter() {
            if TcpListener::bind(("127.0.0.1", *port)).is_err() {
                error!("Port {} is already in use or lacking permission. Exiting...", port);
                return true;
            }
        }
        false
    ...
    pub async fn run_validations(&self, proxy: &str, url: &str, config: Option<&str>) -> bool {
        ...
        let json_contents = fs::read_to_string("src/config/verification_config.json").expect("Failed to read config file");
        let toml_contents = fs::read_to_string("src/config/severity_config.toml").expect("Failed to read config file");
